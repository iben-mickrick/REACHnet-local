**ADAPTABLE PHENOTYPE Iteration 1b. 
...
modifications are included to meet local exclusions. This code differs from iteration 1a with regard to reference dates. This phenotype was run
on a newly refreshed datamart, requiring an update to the dates used to identify eligibility criteria. 

Additional criteria added to this phenotype are as follows:
1. ICD9 & ICD10 diagnostic codes for atrial fibrillation 
2. ICD9 & ICD10 diagnostic codes for long-term anticoagulation use 
3. Provider_ids of clinicians participating in the ADAPTABLE trial --for the purposes of GitHub, these provider_ids are removed 

All other eligibility criteria remained untouched. 

Important eligibility criteria 
1.EligASCVD is defined as MI OR PCI OR CABG 
2.Enrichment factor new is defined as at least one of the following conditions: 65 (and up), diabetes, CVD, PAD, Heart Failure, 
hypertension, hyperlipidemia, smoker (Creatinine enrichment factor is NOT included in this phenotype iteration) 
3.EligCDM is defined as in reference population, not dead, 18 (and up), in eligASCVD, no safety factors and not on contraindicated medication 
*;
...
%let CDM =E:\Ochsner\CDM5\;
%let OUTLIB =E:\ADAPTABLE\Phenotype_0803\;
...
%let REF_END_DT   = '15JUN2016'd;

%let HISTORY_INC_START_DT ='01JAN2010'd;
%let HISTORY_INC_END_DT='15JUN2016'd;

%let HISTORY_EX_START_DT='16JUN2011'd;
%let HISTORY_EX_END_DT='15JUN2016'd;

%let RECENT_START_DT = '16JUN2015'd;
%let RECENT_END_DT   = '15JUN016'd;

%let RX_START_DT = '16DEC2015'd;
%let RX_END_DT   = '15JUN2016'd;

%let LAB_START_DT = '16DEC2015'd;
%let LAB_END_DT   = '15JUN2016'd;

%let AGE_ASOF_DT = '27JUL2016'd; *change to date that new code is run;
...
%let asp_CUI= %nrstr('212033', '892582', '318272', '213269', '243670', '464505', '1424698', '4610129', '783389', '38593');
...
%macro main;
    ...
    |  type IP, EI, or AV                                            
		Provider IDS were removed for posting code to GitHub
																			*/
...
/*Medication:aspirin*/
    ...
    data outlib.adaptable_prelim2;
        ...
        label
			...
			medAsp="Medication/OAC: Aspirin [CDM basis]"
    ...
    /* Check results */
    proc contents data=outlib.adaptable_prelim2;
    run;

    proc freq data=outlib.adaptable_prelim2;
