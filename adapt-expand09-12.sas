**ADAPTABLE PHENOTYPE Iteration 2a. 
...
Reference dates were not changes from iteration 1b. In other words, the reference date in iteration 2a. are the same as those used in iteration
1b. 
...
Contraindicated medications still included in the phenotype but not necessarily always used to calculate eligibility flags 
1.	Warfarin 
2.	Dabigatran
3.	Rivaroxaban
4.	Apixaban
...
%macro main;
    ...
    |  type IP, EI, or AV         
		Provider IDs removed for posting to GitHub
																			*/
 ...
 %checkDx(
	...
	/*Enrichment factor: male sex*/
	%if %upcase (&has_demographic)=Y %then %do;
		proc sql;
			create table enrichSex as
			select distinct cdm_demo.patid
			from cdm.demographic cdm_demo
			where sex='M'
			order by cdm_demo.patid;
			quit;
		%end;
		%else %do;
		data enrichSex;
	...
	/*Enrichment factor: BMI<20kg/m2*/

	%if %upcase (&has_vital)= Y %then %do;
		proc sql;
		create table enrichbmi as 
		select distinct cdm_vit.patid
		from cdm.vital cdm_vit
		where cdm_vit.original_bmi <20.00 and cdm_vit.original_bmi>=15.00
		order by cdm_vit.patid;
		quit;
	%end;
	%else %do;
    ...
    data outlib.adaptable_prelim18;
        merge
            refPop (in = _refPop)
			died (in = _died)
			...
			enrichSex (in = _enrichSex)
			enrichbmi (in = _enrichbmi)
		...
		enrichSex= _enrichSex
		enrichbmi=_enrichbmi;
...
if  refPop and
        ...
        label
			...
			enrichSex = "Enrichment factor: Male [CDM basis]"
    ...
    /* Check results */
    proc contents data=outlib.adaptable_prelim18;
    run;

    proc freq data=outlib.adaptable_prelim18;
