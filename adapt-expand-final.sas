**ADAPTABLE PHENOTYPE Iteration 4a. 
...
3.  Enrichment factor reflects the old, original protcol and is defined as at least one of the following conditions:
	a. 65 (and up) "Age65" 
	b. Diabetes "enrichDiab" 
	c. PAD "enrichPAD"
	d. LVSD "enrichLVSD"
	e. CVD "enrichCVD"
	f. Smoker "enrichSmoker"
	g. Creatinine>=1.5mg/dL 

3.	Enrichment factor1 reflects the recent amendment changes and is defined as at least one of the following conditions: 
	a. 65 (and up)"Age65"
	b. Diabetes "enrichDiab"
	c. CVD "enrichCVD"
	d. PAD "enrichPAD"
	...
	f. LVSD "enrichLVSD"
	g. hypertension "enrichHTN"
	h. LDL>=130mg/dL "enrichHPD"
	i. smoker "enrichSmoker"
	j. Creatinine>=1.5mg/dL "enrichCreat"
...
6.	EligibleCDM2b: is defined as in reference population, not dead, in ASCVD1 and at least 1 enrichment factor1
;
*******************************;
...
Libname CDM 'E:\Ochsner\CDM7\';
run;
libname OUTLIB 'E:\ADAPTABLE\Phenotype-11-16\';
run;

%let CDM =E:\Ochsner\CDM7\;
%let OUTLIB =E:\ADAPTABLE\Phenotype-11-16\;
...
%let CAD_I9_DX= %nrstr ('411.0', '411.1', '411.81', '411.89', '413.0', '413.9', '414.00', '414.01', '414.02', '414.03', '414.04', '414.05', '414.06', '414.07', '414.2', '414.3', '414.4', '414.8', '414.9');

%let CAD_I10_DX= %nrstr ('I20.0', 'I20.8', 'I20.9', 'I24.0', 'I24.1', 'I24.8', 'I24.9', 'I25.10', 'I25.110', 'I25.111', 'I25.118', 'I25.119', 'I25.5', 'I25.6', 'I25.700', 'I25.701', 'I25.708', 'I25.709', 'I25.710', 'I25.711', 'I25.718', 'I25.719', 'I25.720', 'I25.721', 'I25.728', 'I25.729', 'I25.730', 'I25.731', 'I25.738', 'I25.739', 'I25.750', 'I25.751', 'I25.758', 'I25.759', 'I25.760', 'I25.761', 'I25.768', 'I25.769', 'I25.790', 'I25.791', 'I25.798', 'I25.799', 'I25.810', 'I25.811', 'I25.812', 'I25.82', 'I25.83', 'I25.84', 'I25.89', 'I25.9');
...
%let CVD_I10_DX = %nrstr('I09.81', 'I11.0', 'I13.0', 'I13.2', 'I50.1', 'I50.20', 'I50.21', 'I50.22', 'I50.23', 'I50.30', 'I50.31', 'I50.32', 'I50.33', 'I50.40', 'I50.41', 'I50.42', 'I50.43', 'I50.9');

%let PAD_I9_DX = %nrstr('440.20', '440.21', '440.22', '440.23', '440.24', '440.29', '443.9');

%let LVSD_I9_DX = %nrstr('428.20', '428.21', '428.22', '428.23', '428.40', '428.41', '428.42', '428.43');

%let LVSD_I10_DX = %nrstr('I50.20', 'I50.21', 'I50.22', 'I50.23', 'I50.40', 'I50.41', 'I50.42', 'I50.43');
...
%let CHF_I9_DX = %nrstr('398.91', '402.01', '402.11', '402.91', '404.01', '404.03', '404.11', '404.13', '404.91', '404.93', '428.0', '428.1', '428.20', '428.21', '428.22', '428.23', '428.30', '428.31', '428.32', '428.33', '428.40', '428.41', '428.42', '428.43', '428.9');
 ...
 %checkDx(
    ...
    %if %upcase(&has_vital) = Y %then %do;
        proc sql;
            ...
            where cdm_vit.smoking in ('01', '02','05', '07', '08')
    ...
    %end;
		...
		/* Enrichment factor: Creatinine>=1.5mg/dL*/
	%if %upcase(&has_lab_result_cm) = Y %then %do;
        proc sql;
            create table enrichcreat as
            select distinct cdm_lab.patid
            from cdm.lab_result_cm cdm_lab
            where
				cdm_lab.lab_loinc='2160-0' and
				cdm_lab.result_num >= 1.5 and 
				cdm_lab.result_date between &lab_start_dt and &lab_end_dt
    ...
    %end;
			...
			/* Enrichment factor: LDL>=130mg/dL*/
	%if %upcase(&has_lab_result_cm) = Y %then %do;
        proc sql;
            create table enrichldl as
            select distinct cdm_lab.patid
            from cdm.lab_result_cm cdm_lab
            where
				cdm_lab.lab_loinc='2089-1' and
				cdm_lab.result_num >= 130 and 
				cdm_lab.result_date between &lab_start_dt and &lab_end_dt
            order by cdm_lab.patid;
        quit;
    %end;
    ...
    data outlib.adaptable_prelim;
        merge
			...
			enrichcreat (in= _enrichcreat)
			enrichldl (in= _enrichldl)
		...
		Eligibility flag to use if IRB amendment not approved 
		*/
        if  refPop and
            not died and
            eligAge18 and
		...
		/*set secondary eligible flag
			CDM2a=ASCVD1 with enrichment factor 
			CDM2b=ASCVD1 with enrichment factor1--the flag to be used if IRB amendment approved  
			*/
...
%main
 ...
 proc freq data=outlib.adaptable_prelim;
        title "ADAPTABLE Phenotype CDM / Preliminary eligibility count";
        tables eligibleCDM eligibleCDM2a eligibleCDM2b;
    run;
